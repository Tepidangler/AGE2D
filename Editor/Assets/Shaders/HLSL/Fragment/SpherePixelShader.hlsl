...
Texture2D MatTextures[4] : register(t1);

const float PI = 3.14159265359;

float3 GetNormalFromMap(PS_OUTPUT Input)
{
    float3 TangentNormal = mul(MatTextures[1].Sample(Sampler0, Input.UV).xyz, 2.f) - 1.f;
    
    float3 Q1 = ddx(Input.WorldPosition);
    float3 Q2 = ddy(Input.WorldPosition);
    float2 ST1 = ddx(Input.UV);
    float2 ST2 = ddx(Input.UV);
...
float DistributionGGX(float3 X, float3 H, float Roughness)
{
    ...
    float Nom = A2;
    float Denom = mul(NdotH2, (A2 - 1.f)) + 1.f;
...
{
    return mul(F0 + (1.f - F0), pow(saturate(1.f - CosTheta, 0.f, 1.f), 5.f));
}

PS_OUTPUT SimplePixelShader(PS_OUTPUT input) : SV_TARGET
{
    ...
    float3 Albedo = pow(MatTextures[0].Sample(Sampler0, output.UV).rgb, float3(2.2f));
    float Metallic = MatTextures[2].Sample(Sampler0, output.UV).r;
    float Roughness = MatTextures[3].Sample(Sampler0, output.UV).r;
    float AO = MatTextures[4].Sample(Sampler0, output.UV).r;
    
    float3 N = GetNormalFromMap();
    float3 V = normalize(float3(0.f) - input.WorldPosition);
    
    float3 F0 = flaot3(0.04f);
    
    F0 = lerp(F0, Albedo, Metallic);
    
    float3 Lo = float3(0.f);
    float3 ambient = mul(mul(float3(0.03f), Albedo), AO);
